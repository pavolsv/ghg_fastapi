...
async def factor_management(
        ...
        with Session(engine) as session:
            ...
            if name:
                gwp_query = gwp_query.filter(
                    (GWPReference.gas_name_zh.contains(name)) |
                    (GWPReference.gas_name_en.contains(name)) |
                    (GWPReference.formula.contains(name))
                )
            if gas_type:  # 前端 gas_type 下拉重新利用為 version 篩選
                gwp_query = gwp_query.filter(GWPReference.version == gas_type)
            gwp_factors = gwp_query.order_by(GWPReference.formula.asc()).all()
            version_list = sorted(
                {v[0] for v in session.query(GWPReference.version).distinct().all() if v[0]}
            )
        return templates.TemplateResponse(
    ...
    with Session(engine) as session:
        ...
        # --- 篩選邏輯（配合分組顯示調整）---
        if name:
            query = query.filter(EmissionFactor.name.contains(name))
        if category:
            query = query.filter(EmissionFactor.emission_type.contains(category))
        if year and year.strip():
            try:
                query = query.filter(EmissionFactor.year == int(year))
            except ValueError:
                pass
        if gas_type:
            ...
            matching_codes = [
                row[0]
                for row in session.query(EmissionFactor.code)
                .filter(EmissionFactor.gas_type == gas_type)
                .distinct()
                .all()
            ]
            query = query.filter(EmissionFactor.code.in_(matching_codes))

        # --- 核心修改：預設按 Code (編號) 升冪排序 (A-Z) ---
        factors = query.order_by(
            EmissionFactor.code.asc(), EmissionFactor.year.desc()
        ).all()
        ...
        # 獲取選單資料
        name_list = [
            n[0] for n in session.query(EmissionFactor.name).distinct().all() if n[0]
        ]
        gas_list = [
            g[0]
            for g in session.query(EmissionFactor.gas_type).distinct().all()
            if g[0]
        ]
