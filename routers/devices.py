...
def _get_user_id(request: Request) -> Optional[int]:
    """從 session 取得當前使用者 ID，未登入回傳 None"""
...
async def list_device_activities(
    ...
    try:
        ...
        devices = session.exec(
            select(Device).where(Device.account_id == user_id).order_by(col(Device.id))
        ).all()
        device_ids = [d.id for d in devices]
...
async def save_device_activity(
    ...
    try:
        ...
        if device.account_id != user_id:
            return JSONResponse(
                status_code=403,
                content={"success": False, "message": "無權限操作此設備"}
            )
